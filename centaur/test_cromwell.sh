...
usage="
$PROGNAME [-h] [-b branch] [-j jar path] [-r rundir] [-c config file] [-t refresh token file] [-s] [-e excludeTag] [-i testDirPath]
...
Arguments:
    ...
    -s    If supplied, use the file cromwell-service-account.json for various centaur operations
...
while getopts ":hb:r:c:p:j:t:se:i:" option; do
    case "$option" in
        h) echo "$usage"
            exit
            ;;
        ...
        t) REFRESH_TOKEN=-Dcentaur.optionalTokenPath="${OPTARG}"
            ;;
        s) CENTAUR_GOOGLE="-Dcentaur.google.auth=service-account"
           # For now, the json file is specified in centaur's reference.conf.
           # Ideally one could pass a path to a service account json file. This script currently builds up a config on
           # the command line instead of in a file. HOCON does not support creating command line arrays-of-objects.
            ;;
